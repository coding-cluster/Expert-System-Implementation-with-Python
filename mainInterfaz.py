...
class ExpertSystemWindow(QMainWindow):
    ...
    def load_csv(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Selecciona un archivo CSV", "", "CSV Files (*.csv)")
        if file_path:
            try:
                self.rule_set = pd.read_csv(file_path)
                self.target_column = self.rule_set.columns[0]
                self.output_text.append(f"CSV cargado: {os.path.basename(file_path)}")
                self.output_text.append(f"Columna Objetivo: {self.target_column}")
            except Exception as e:
                self.output_text.append(f"Error cargando CSV: {str(e)}")

    def start_new_request(self):
        if self.rule_set is None:
            self.output_text.append("Por favor selecciona un archivo primero")
            return
        ...
        if selected_method == "Forward-Chaining":
            ...
            self.setup_forward_chaining(columna)
    ...
    def setup_forward_chaining(self, columna):
        ...
        all_attributes = set()
        for attributes in self.rule_set.iloc[:, columna:].values:
        ...
        if self.submit_button is None:
            ...
            self.submit_button.clicked.connect(self.run_forward_chaining(columna))
            self.layout.addWidget(self.submit_button)

    def run_forward_chaining(self,columna):
        ...
        for _, row in self.rule_set.iterrows():
            ...
            attributes = set(row.dropna().values[columna:])
